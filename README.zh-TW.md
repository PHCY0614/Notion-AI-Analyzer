...
它會分析頁面中的純文字，產生正體中文標題、摘要、關鍵字與暫定主題，再將結果寫回 Notion。

英文版：[`README.md`](README.md)

使用者介面採用符合臺灣語言習慣的正體中文；程式碼與開發文件主要以英文維護。
...
設定頁不顯示或編輯分析提示詞，只提供輸出規格與單次 AI 等待上限的調整。你可以按「恢復預設規格」回到本版本的預設輸出設定。
...
本擴充功能沒有額外的打包流程。在 Chrome 載入未封裝項目時，會直接使用倉庫根目錄的檔案。

開發與檢查需要 Node.js 18 或更新版本。複製倉庫後，在專案根目錄執行：

```bash
npm test
```

這會執行 `tests/` 底下既有的 Node 測試（使用 Node 內建的 `assert`，不需安裝測試套件）。`npm run check` 目前與 `npm test` 相同，可用來做一次完整檢查。
