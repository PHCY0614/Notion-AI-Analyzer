...
目前使用者介面是為台灣使用者設計，使用符合台灣用語與語言習慣的繁體中文；程式碼與開發者文件則主要以英文維護。
...
- 主題整理查詢只要求回傳 `整理狀態`、`AI 暫定主題` 與 `AI 主題` 三個資料庫欄位。不把頁面正文、AI 標題、摘要或關鍵字送給主題分類，也不把篇數、共現關係或文章相似度當成分類依據。
...
使用本工具前，資料庫必須已有 `整理狀態` 選取欄位及 `待分析` 選項。測試連線、掃描、開始佇列，或在 Notion 頁面開啟 Popup 時，會先檢查這兩項（`整理狀態` 必須是 select 且包含 `待分析`）；缺少時停止並直接說明如何在 Notion 修正。守門通過後，工具才可能 PATCH 補上缺少的 AI 欄位與其餘流程狀態；不會刪除、移動、重新命名或改動使用者自行建立的其他選項。
...
透過 Google AI Studio 或 Gemini API 未付費服務提交的內容與回應，可能由 Google 用於改善與開發其產品、服務及機器學習技術，也可能由人工檢閱。請勿提交敏感、機密或個人資訊。若專案已啟用 Cloud Billing，可能適用付費服務的資料條款；實際規則以 [Gemini API Additional Terms of Service](https://ai.google.dev/gemini-api/terms) 為準。
...
2. 讀取純文字區塊。
3. 一般文章以一次主要 AI 呼叫產生標題、摘要、關鍵字與暫定主題；長文可能先以分段呼叫抽取筆記，再進行整合呼叫。
...
在設定資料庫中的 Notion 文章頁開啟擴充功能，按「分析目前頁面」。工具直接從目前網址辨識 Page ID，不依賴來源網址、貼文編號、擷取鍵或本機佇列順序。若頁面已有 AI 內容，確認視窗會說明再分析將重新產生 AI 分析結果、清除所有既有的 `AI 主題`，並重新開始主題確認。分析後可逐一採用候選、改用既有主題、自訂最多 5 字的新主題，或暫不處理。這次重新確認的主題會以聯集方式寫入 `AI 主題`；暫不處理的候選會留在 `AI 暫定主題`，直到完成處置。全部解決並轉為 `已分析` 後才清空。

## 整理主題

每次按「掃描並產生整理建議」，會依序查詢 `待主題整理` 或 `待主題確認` 頁面，直到收集到最多 75 個去重後的 `AI 暫定主題`，或已沒有更多符合條件的頁面。少於 75 個時照常執行；每次按鈕操作就是獨立的一批，不會自動接續下一批。

主題整理查詢只要求回傳 `整理狀態`、`AI 暫定主題` 與 `AI 主題` 三個資料庫欄位；Page ID 只用於正確寫回。送給 AI 的分類內容只有本批不重複的暫定主題名稱與可參考的既有 `AI 主題` 名稱。頁面正文、名稱、AI 標題、摘要、關鍵字、出現次數、共同出現關係及影響篇數都不參與分類判斷。
...
擴充功能開發者沒有營運獨立的中介伺服器，本擴充功能也沒有分析追蹤或遙測。文章直接從 Notion 傳到你選擇的 AI 服務商；未選的服務商不會收到文章。圖片、影片、音訊、檔案與 PDF 不會作為分析內容。在 Notion 頁面開啟 Popup 可能為辨識目前頁面而查詢 Notion。詳細資料流請見 [`PRIVACY.zh-TW.md`](PRIVACY.zh-TW.md)（[英文](PRIVACY.md)）。
