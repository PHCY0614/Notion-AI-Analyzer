...
本政策適用於透過 Chrome Web Store 或本專案 GitHub 提供的 **Siftly**（AI Content Organiser for Notion）版本。
...
5. 合格結果由擴充功能直接寫回 `api.notion.com`。測試連線時，若缺少 `整理狀態` Select 欄位或 `待分析` 選項，設定頁會先顯示確認視窗；只有使用者選擇「新增並繼續」後，才會建立該欄位或補齊分析流程需要的狀態選項。既有欄位型別不會自動轉換，使用者自建的選項不會被刪除或改名。完成這項準備後，測試連線、掃描、開始佇列，或在 Notion 頁面開啟 Popup 時，可能再 PATCH 補上缺少的 AI 欄位與其餘狀態選項。
