...
## Chrome Web Store 使用者資料承諾

本擴充功能對使用者資料的使用遵守 [Chrome Web Store 使用者資料政策](https://developer.chrome.com/docs/webstore/program-policies/user-data-faq)，包括 Limited Use 規定。所讀取、保存或傳送的資料只用於提供下述 Notion 文章分析、主題整理、結果寫回與使用者啟動的佇列功能，不用於個人化廣告、信用評估或販售，也不會傳送給與這些功能無關的第三方。
...
- 使用者在設定頁按下「載入可用資料庫」時，程式會透過 Notion Search API 讀取已授權 Data Source 的名稱、ID、父 Database ID 與字元型 emoji，以建立資料庫選單；不會保留完整 schema、description、cover、使用者資料或外部圖片網址。
...
1. 擴充功能直接連接 `api.notion.com` 讀取指定頁面。Notion Token 放在 HTTP 標頭，不放在網址。
   設定頁的資料庫清單也由擴充功能直接向 Notion Search API 取得，只有在使用者按下載入或重新整理時才會請求。
...
5. 合格結果由擴充功能直接寫回 `api.notion.com`。測試連線時，若缺少 `整理狀態` Select 欄位或 `待分析` 選項，設定頁會先顯示確認視窗；只有使用者選擇「新增並繼續」後，才會建立該欄位或補齊分析流程需要的狀態選項。既有欄位型別不會自動轉換，使用者自建的選項不會被刪除或改名。完成這項準備後，測試連線（「測試連線並準備欄位」）、掃描或開始佇列時，可能再 PATCH 補上缺少的 AI 欄位與其餘狀態選項。開啟 Popup 檢查目前頁面時不會 PATCH 資料庫結構。
...
- 一般設定：Notion 目標 ID、AI 服務商與模型名稱、關鍵字排除清單、分析提示詞與輸出規格、全域與單頁主題對照記憶及是否記住金鑰。
...
上述本機資料不會同步到開發者營運的帳號或伺服器，開發者無法存取。

設定頁載入的資料庫清單只存在該設定頁的記憶體，不會寫入 Chrome storage；關閉或重新開啟設定頁後需要再次載入。使用尚未儲存的 Token 載入清單時，Token 只用於該次 Notion API 請求，不會因載入清單而保存。
...
如有隱私或資料處理相關問題，可透過本專案的 [GitHub Issues](https://github.com/PHCY0614/siftly/issues) 提出。請勿在公開 Issue 中貼上 API Key、Notion Token、文章內容或其他私人資料。
